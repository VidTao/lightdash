...
export class UserController extends BaseController {
    ...
    async getUserMcpCredentials(@Request() req: express.Request): Promise<{
        ...
        try {
            ...
            const personalAccessTokens = await this.services
                .getPersonalAccessTokenService()
                .getAllPersonalAccessTokens(req.user);
            ...
            if (personalAccessTokens.length > 0) {
                const activeToken = personalAccessTokens.find(
                    token => !token.expiresAt || new Date(token.expiresAt) > new Date()
                );
                apiKey = activeToken?.uuid;
            } else {
                const newToken = await this.services
                    .getPersonalAccessTokenService()
                    .createPersonalAccessToken(
                        req.user,
                        {
